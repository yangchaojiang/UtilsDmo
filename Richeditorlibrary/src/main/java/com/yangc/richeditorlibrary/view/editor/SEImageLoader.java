...
public class SEImageLoader {
			...
			if (bm != null) {
				...
				addTask(new Runnable() {
					...
					public void run() {
						Bitmap bm;
						if (path.contains("http")){
							bm=getHttpBitmap(path,maxSize);
						}else {
							bm = compressBitmap(path, maxSize);
						}
						addBitmapToLruCache(path, bm);
						sendImageLoadCompleteMessage(path, imageView, getBitmapFromLruCache(path));
						mPoolSemaphore.release();
	...
	public Bitmap compressBitmap(InputStream inputStream, PointF maxSize) {
		Bitmap compressBitmap = null;
		// 限定-尺寸
		float wMax = maxSize.x;
		float hMax = maxSize.y;
		...
		if (!opt.mCancel && opt.outWidth != -1 && opt.outHeight != -1) {
			...
			compressBitmap = BitmapFactory.decodeStream(inputStream, null,opt);
		}
		return compressBitmap;
	}
	/**
	 ...
	 * @return
	 */
	public   Bitmap getHttpBitmap(String url,PointF maxSize){
		URL myFileURL;
		Bitmap bitmap=null;
		try{
			...
			if (maxSize==null){
				...
				bitmap=compressBitmap(is,maxSize);
			}
